...
if __name__ == '__main__':
    ...
    parser.add_argument('--root', type=str, default='data/waymo/')
    ...
    datamodule = EPDataModule(root = args.root,
                              ...
                              val_processed_dir='data/waymo/val/processed/',
                              train_batch_size=1,
                              val_batch_size=1,
                              test_batch_size=1,
                              val_subsample_path = args.val_subsample_path,
                              val_transform = TargetBuilder(50, 41)
                              )
    ...
    submission_specs.N_SIMULATION_STEPS = 41
    submission_specs.N_FULL_SCENARIO_STEPS = 91
    ...
    for i, data in enumerate(tqdm(datamodule.val_dataloader())):
        ...
        src_file = 'data/waymo/val/sim_agent/' + scenario_id+'.pkl'
        ...
        phi_b_p = torch.Tensor(basis_function_b(tau=model.tau_pred[1:],
                                                ...
                                                k=1,
                                                return_kron=True,
                                                delta_t = 6.0)).to(denoised_cps.device) # prime of basis function
        ...
        x = pred_trajs[..., 0]
        y = pred_trajs[..., 1]
        gt_trajectories = sim_agent_utils.get_gt_trajectories(scenario) 
        z = gt_trajectories.z  # use ground-truth z, since it is not predicted
        ...
        sim_mask = sim_agent_utils.get_sim_mask_waymo(scenario)
        simulated_states = tf.concat([x[:, sim_mask, :submission_specs.N_SIMULATION_STEPS, None], 
                                      y[:, sim_mask, :submission_specs.N_SIMULATION_STEPS, None], 
                                      np.repeat(z[None, :, :, None], 32, axis=0), 
                                      heading[:, sim_mask, :submission_specs.N_SIMULATION_STEPS, None]], axis=-1)
