'''
Copyright (C) 2025 co-pace GmbH (a subsidiary of Continental AG).
Licensed under the BSD-3-Clause License.
@author: Yao Yue
'''

from typing import Callable, Optional

import pytorch_lightning as pl
from torch_geometric.loader import DataLoader

from datasets import EPDataset
from transforms import TargetBuilder
import torch

class EPDataModule(pl.LightningDataModule):

    def __init__(self,
                 root: str,
                 dataset: str,
                 train_batch_size: int,
                 val_batch_size: int,
                 test_batch_size: int,
                 shuffle: bool = True,
                 num_workers: int = 0,
                 pin_memory: bool = True,
                 persistent_workers: bool = True,
                 train_raw_dir: Optional[str] = None,
                 val_raw_dir: Optional[str] = None,
                 test_raw_dir: Optional[str] = None,
                 train_processed_dir: Optional[str] = None,
                 val_processed_dir: Optional[str] = None,
                 test_processed_dir: Optional[str] = None,
                 train_transform: Optional[Callable] = TargetBuilder(50, 60),
                 val_transform: Optional[Callable] = TargetBuilder(50, 60),
                 test_transform: Optional[Callable] = None,
                 val_subsample_path: Optional[str] = None,
                 **kwargs) -> None:
        super(EPDataModule, self).__init__()
        self.root = root
        self.dataset = dataset
        self.train_batch_size = train_batch_size
        self.val_batch_size = val_batch_size
        self.test_batch_size = test_batch_size
        self.shuffle = shuffle
        self.num_workers = num_workers
        self.pin_memory = pin_memory
        self.persistent_workers = persistent_workers and num_workers > 0
        self.train_raw_dir = train_raw_dir
        self.val_raw_dir = val_raw_dir
        self.test_raw_dir = test_raw_dir
        self.train_processed_dir = train_processed_dir
        self.val_processed_dir = val_processed_dir
        self.test_processed_dir = test_processed_dir
        self.train_transform = train_transform
        self.val_transform = val_transform
        self.test_transform = test_transform
        self.val_subsample_path = val_subsample_path

    def setup(self, stage: Optional[str] = None) -> None:
        self.train_dataset = EPDataset(root=self.root, 
                                       dataset = self.dataset,
                                       split='train', 
                                       processed_dir=self.train_processed_dir,
                                       transform= self.train_transform,
                                      )
        
        self.val_dataset = EPDataset(root = self.root, 
                                     dataset = self.dataset,
                                     split = 'val',  
                                     processed_dir = self.val_processed_dir,
                                     transform= self.val_transform,
                                     subsample_file_path = self.val_subsample_path
                                    )
        
        # self.test_dataset = EPDataset(root=self.root, 
        #                                        split='test', 
        #                                        processed_dir= self.test_processed_dir,
        #                                        transform= self.test_transform
        #                                       )
        

    def train_dataloader(self):
        return DataLoader(self.train_dataset, batch_size=self.train_batch_size, shuffle=True,
                          num_workers=self.num_workers, pin_memory=self.pin_memory,
                          persistent_workers=self.persistent_workers,
                          )


    def val_dataloader(self):
        return DataLoader(self.val_dataset, batch_size=self.val_batch_size, shuffle=False,
                          num_workers=self.num_workers, pin_memory=False,
                          persistent_workers=self.persistent_workers,
                         )


    # def test_dataloader(self):
    #     return DataLoader(self.test_dataset, batch_size=self.test_batch_size, shuffle=False,
    #                       num_workers=self.num_workers, pin_memory=self.pin_memory,
    #                       persistent_workers=self.persistent_workers)