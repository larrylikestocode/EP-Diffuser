...
def topk_joint(max_guesses: int,
               pred: torch.Tensor,
               prob: Optional[torch.Tensor] = None,
               ptr: Optional[torch.Tensor] = None,) -> Tuple[torch.Tensor, torch.Tensor]:
    '''
        pred: [K, A, 60, 2]
        target: [A, 60 ,2]
        prob: [B, K]
    ...
    max_guesses = min(max_guesses, pred.size(0))
    if max_guesses == pred.size(0):
        return pred, prob
    
    prob_topk, inds_topk = torch.topk(prob, k=max_guesses, dim=-1, largest=True, sorted=True) # [B, max_guesses]
    
    inds_topk = gather_csr(src=inds_topk, indptr=ptr) # [A, max_guesses]
    pred_topk = pred[inds_topk.transpose(0,1), torch.arange(pred.size(1)).unsqueeze(0).expand(max_guesses, -1)]
